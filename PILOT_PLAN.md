...
- **H3 · Calendar export + contract display + reviews frontend (C)** — `.ics` (participant + creator) + post-publish contract view + the review surface (backend done in v5.1).
  - **Reviews (C):** auto-prompt to **rate the experience** on the post-session screen when the experience's **last** session ends (peak engagement; mid-experience sessions never prompt → no review fatigue); solo sessions prompt after they end. Persist as a re-ask — a standing "Rate your experience" card in the Experience Space while `member && experience_review_open && !reviewed` (optional later: a one-time `review_request` notification nudge). Buyer-page **social proof**: experience rating (lineage-cumulative, `vw_experience_review_stats`) + Expert rating (`app_profile_public`), each gated to ≥1 review; optional "N members" tribe count. Calls `upsert_experience_review` / `upsert_solo_session_review`. Minor follow-up: buyer-page "this experience has concluded" state for `completed` experiences (or fold into H5).
