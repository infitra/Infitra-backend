...
### Architecture deltas (v5.1 — DONE this cycle, ahead of H2)
Discovered mid-Phase-2 during a notifications pass; cleanup + a review-model correction, settled together before resuming H2.
- **Badge system fully removed** (DB + frontend + edge fn): ~30 objects (4 tables, 21 functions, 8 views, triggers, the `badge_awarded`/`badge_monthly_digest` notification types, the `monthly_badges` cron fn). It was unused (zero frontend), its catalog seed was never in git, and a badge trigger on `app_attendance` could roll back **paid entitlement** (the landmine — neutralized first). Full DDL preserved in git history for clean-slate reintroduction. **Tribe (`app_creator_space*`) KEPT** — the purchase-driven audience primitive the Experience Space depends on.
- **Follow dead-code removed**: the phantom-`app_follow` `follow_user`/`unfollow_user` edge fns, orphaned `FollowButton`, and inert `join_creator_space`/`leave_creator_space` RPCs. The real "follow" = tribe membership; the **continuation view-only model is already wired** (`can_post_in_challenge_space` + `current_active_challenge_in_group`: posting requires having bought the *active* experience; after a continuation, prior members keep view+like but can't post without buying the new one).
- **Reviews reshaped to match the product** (backend done): `app_review` is now **product-scoped** (one review per experience OR per solo session, XOR-constrained). **Completion lifecycle** added (`complete_ended_experiences` + daily pg_cron): `status='completed'` at `end_date` stops new purchases and marks "past" — **decoupled from reviews and from tribe posting**. Review eligibility = member + `experience_review_open()` (all sessions ended **OR** end_date passed) → unlocks at the last-session moment. **Aggregates**: Expert rating fans out to owner + co-hosts (full credit) + solo host/cohosts (`vw_expert_review_stats` → `app_profile_public`); experience rating rolls up continuation lineage (`vw_experience_review_stats`). Collab reviews gated to after the experience ends. **Frontend = C (in H3).**
...
- **DROPPED (deliberate):** ~~capture payout destination / record payouts in `app_payout`~~. No IBAN in DB (sensitive PII **and** not Connect-forward-compatible — Connect keeps bank details on Stripe's side, so DB columns would be throwaway); no `app_payout` recording. Payout details collected **off-DB** + paid manually until Stripe Connect.
- ✅ Earnings page rebuilt **co-host-aware** (SECURITY DEFINER `vw_my_earnings_lines`): shows each creator's **net** (owner remainder + co-host income), per-sale `gross → platform → co-host → your cut` with per-row effective rate + totals reconciliation; Stripe fees removed (buyer-absorbed). Co-host splits are now **surfaced in-app** (were "manual at payout"). Shows **net earned**, not accrued/owed-vs-paid (no payout records to diff — fine for a single end-of-pilot payout).
...
### Phase 2 — final order
- **H0 · LiveKit spike — ✅ DONE (clean).** Confirmed `npm:livekit-server-sdk@^2` mints a valid, correctly-granted LiveKit JWT in the Supabase Deno edge runtime (Deno 2.1.4; cold start incl. npm fetch ≈2.5s; no Node-API friction). → **Daily→LiveKit migration is technically GREENLIT but DEFERRED to post-pilot** (~3.5–5d; pilot stays on Daily — see Parked). What remains is execution, not unknown: a free LiveKit Cloud project (wss URL + API key/secret as edge secrets) + token-mint in `issue_join_token` + webhook remap + `LiveRoomEmbed` → `<LiveKitRoom><VideoConference/>` + room create/end.
...
- **H2 · Pilot economics + earnings — ✅ DONE (leaned).** Configurable founding 10% fee end-to-end (verified live); co-host-aware earnings page (`vw_my_earnings_lines`) showing net + per-sale breakdown. Payout-destination capture + `app_payout` recording **dropped** (off-DB / manual until Connect).
- **H3 · Calendar export + contract display + reviews frontend (C) — ✅ DONE (2026-06).**
  - **H3a `.ics` calendar export** ✅ — participant (per-experience, member-gated) + creator (hosting) routes; `INFITRA:` titles, experts in the description, published-only, "Exported at" stamp. (`lib/ics.ts`, `lib/challenges/sessionExperts.ts`, two `route.ts`, `CalendarButton`.)
  - **H3b post-publish contract view** ✅ — read-only render of the frozen, hashed `snapshot_json` at `/dashboard/collaborate/[id]/contract` (owner+cohost; RLS-gated): parties + revenue split, terms, schedule, **signatures with exact timestamps** (owner = lock; co-host = `app_collaboration_acceptance.accepted_at`), SHA-256 tamper-evidence, zone-labelled times (UTC canonical). Linked from the published page + the dashboard's repointed "View contract" chip. (`ContractDocument`.)
  - **H3c reviews** ✅ — both review types, role-aware in the Space, gated `experience_review_open`:
    - **① Experience review (participant → experience):** `ReviewCard` → `upsert_experience_review`; one rating credits all creators (`vw_expert_review_stats`).
    - **② Collaboration review (creator → co-host):** `CollabReviewCard` → `collab_review_create`; **creator-only** ("Private · Creators only"), never shown to participants (RLS enforces the read side).
  - **Deferred (noted):**
    - **Buyer-page review social proof** (experience + Expert ratings, public review lists) → **pending continuation design** — ratings are lineage-cumulative (`continuation_group_id`); how they carry across continued experiences must be designed first.
    - **Solo-session review** (`upsert_solo_session_review`) → deferred (pilot = collaborative experiences).
    - **Collab-reputation read** (a creator viewing collab reviews written *about* them, `collab_reviews_for_creator`) → with the lean **creator/participant profiles** work.
    - **Purchase lock (was "concluded")** → re-scoped: **once the first session has started, the experience can no longer be bought** (enforce on the buyer-page CTA + `create_checkout_session`). Deferred.
  - **Verification:** ①+② can't be seen on current data (no experience the test user joins is `experience_review_open` yet). **Folded into the continuation test pass** — one test experience driven to completion verifies the continuation flow **and** the review cards (①+②) together.
- **H4 · Locked-state buyer preview (LEANED from "two-sided workspace").** At the locked/pre-publish state, render the REAL buyer page from the draft so creators see the published product *before* pulling the trigger — reusing the buyer-page blocks already shared with the post-publish success page (so it's wiring + an entry point, not a rebuild). One build-time confirm: whether `vw_challenge_buyer_view` returns draft rows; if not, read the draft tables directly. Full live-updating two-pane preview deferred post-pilot.
...
- **Parked (triggers):** Stripe Connect + tiers · native app · **Daily→LiveKit migration** — spike was clean, but the `live_provider` adapter keeps the swap cheap whenever we do it, so it's **deferred to post-pilot** (trigger: scale cost or branded in-room UI). Pilot runs on **Daily** — no fresh live-stack risk under a flagship pilot for zero pilot-scale benefit. *(Continuation moved OUT of parked → INTO the pre-pilot build; see principle below.)*

### Pilot sequencing principle (locked 2026-06-10)
**Build everything before launch; freeze feature-building during the pilot.** The full pre-pilot set ships first — H3 (incl. **reviews**), H4-lean preview, H5 polish, Bundle 10 emails, **continuation flow**, Bundle 11 verification, H6 landing — so that *during* the pilot the focus is running it + catching downfalls/errors, not building. "Freeze" = no new features; **bug-fixing / hardening is expected.** First creators are flagship reference partners expected to continue → quality-first on core-loop surfaces (publish-preview, contract, dashboard, live, earnings), scope-guarded against blanket polish. Caveat: completion-tail features (review prompts, continuation) are **built + simulated** pre-pilot but have their first **real** firing once a pilot experience completes — that's observation, not building.
