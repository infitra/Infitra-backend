...
- **Database SCHEMA — 🔴 real gap.** The `supabase/migrations/` folder (44 files) does **not** fully reproduce the schema: core objects (`app_transaction`, `app_payout`, `vw_my_creator_summary`, `vw_my_transactions`, and other base tables) **pre-date migration tracking**. There is **no full schema dump in the repo**. → If the Supabase DB were lost, **git cannot rebuild it**; you depend entirely on Supabase's backups. This is the #1 fix.
