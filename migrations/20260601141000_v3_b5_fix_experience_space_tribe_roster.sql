-- Bundle 5a fix — the Tribe roster must come from app_challenge_member (the real
-- enrollment, written by the Stripe webhook), NOT app_challenge_space_member,
-- which is dead schema (0 rows, nothing ever writes it). load_experience_space
-- was showing an empty Tribe for properly-enrolled members.
--
-- Only the `members` + `member_count` slices change; access + is_member already
-- key off app_challenge_member (correct).
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- FIX: Tribe roster from app_challenge_member (the real enrollment).
    'members', COALESCE((
