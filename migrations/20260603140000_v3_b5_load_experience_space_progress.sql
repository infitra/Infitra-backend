-- Bundle 5c (locker-room v5) — add a `progress` slice to load_experience_space
-- so the Experience Space can show real, personal progress: how many sessions
-- you've attended, your attendance %, completion %, and what's still ahead.
--
-- Data comes from vw_my_challenges_progress (per-user, per-challenge; filters by
-- the caller internally). Read inside this SECURITY DEFINER function, scoped to
-- p_challenge_id, so it stays part of the one consolidated round-trip. NULL when
-- there's no progress row yet (e.g. not enrolled) — the client defaults to zeros.
--
-- Only the `progress` key is added; every other slice is unchanged from
-- 20260603120000 (viewer slice).
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- NEW: personal progress for this experience (attendance + completion).
    'progress', (
      SELECT jsonb_build_object(
