-- Bundle 5c (locker-room v2) — add a `viewer` slice to load_experience_space so
-- the Experience Space can render a personalized header ("you in this
-- experience"): your avatar + name, when you joined, and how many times you've
-- posted to the Tribe. Everything is scoped to the calling user (v_user) inside
-- the existing SECURITY DEFINER function, so no new read surface is exposed.
--
-- Only the `viewer` key is added; every other slice is unchanged from
-- 20260601141000 (Tribe roster fix).
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'space_id', v_space.id,
    -- NEW: the calling user, for the personalized header. Scoped to v_user.
