/**
 * PublicChallengeHeroWeekly — Bundle 4.2.35.
 *
 * A/B experiment variant of PublicChallengeHero. Identical in every
 * respect EXCEPT the SESSIONS region: this version uses the
 * WeeklyJourneyCarousel (W1/W2/W3 navigation + per-week sessions
 * list, resurrected from 4.2.13) instead of the flat sessions
 * carousel. Lives at the `/challenges/[id]/weekly` route.
 *
 * The whole point of this experiment is to isolate one variable:
 * does a nested weekly navigation feel better to buyers than a flat
 * scrollable row of sessions? Everything else (page hero, eyebrow,
 * H1, cover image, experts portraits, stats line, dates, CTA pill,
 * positioning sentence) is held constant against the main page so
 * feedback is unambiguously about the sessions section.
 *
 * Lifecycle: kept until feedback decides flat vs weekly. Winner is
 * promoted to the main route; loser is deleted (this file + the
 * weekly page route + WeeklyJourneyCarousel.tsx + any imports).
 *
 * For any other page-level polish (typography, CTA, etc.), edit
 * PublicChallengeHero.tsx and mirror the change here. The two
 * variants should NEVER drift on anything except the sessions
 * region; otherwise the comparison stops being about the carousel.
 */
...
interface Props {
  ...
  currency: string;
  creators: Creator[];
  /** Variant-specific: the WeeklyJourneyCarousel consumes the weeks
   *  shape directly (one slide per week with that week's sessions
   *  listed). The flat-carousel variant uses `sessions` instead. */
  weeks: WeekData[];
...
export function PublicChallengeHeroWeekly({
  challengeId,
  spaceId,
  title,
  promise,
