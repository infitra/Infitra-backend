 ...
 * frozen snapshot (legal names, revenue split, schedule, hash). Signing times
 * come live from app_collaboration_acceptance (co-hosts) + the contract
 * locked_at (owner). The pre-publish workspace IS the contract review surface,
 * but it's gone after publish — this is the durable single source of truth.
 */
export function ContractDocument({
  ...
  lockedAt: string;
  sha256: string | null;
  /** cohost profile_id → acceptance timestamp (ISO). */
  ...
  const lockedBy = snapshot.audit?.locked_by ?? null;
  const lockedByName = lockedBy ? nameById.get(lockedBy) ?? null : null;

  // Signatures, in party order (owner first). The owner's signature is the
  // lock; each co-host's is their acceptance.
  const signers = parties.map((p) => {
    const isHost = p.role === "host" || p.profile_id === lockedBy;
    const signedAt = isHost ? lockedAt : acceptedAt[p.profile_id] ?? null;
    return { profile_id: p.profile_id, name: p.legal_name ?? "—", isHost, signedAt };
  });
  ...
  return (
    ...
    >
      ...
      {/* Signatures — the signing block, like a contract's signature page. */}
      <Section label="Signatures">
        <div className="space-y-2.5">
          {signers.map((s) => (
            <div
              key={s.profile_id}
              className="rounded-2xl px-4 py-3.5 flex items-end justify-between gap-3"
              style={{
                border: "1px solid rgba(15,34,41,0.08)",
            ...
            >
              <div className="min-w-0">
                <p
                  className="text-base font-black font-headline inline-block"
                  style={{
                    color: INK,
                    borderBottom: "1px solid rgba(15,34,41,0.20)",
                    paddingBottom: "2px",
                  }}
                >
                  {s.name}
                ...
                <p
                  className="text-[10px] font-bold uppercase tracking-wider font-headline mt-1.5"
                  style={{ color: s.isHost ? ORANGE : CYAN }}
                >
                  {s.isHost ? "Owner / Host" : "Co-host"}
                </p>
              </div>
              <div className="text-right shrink-0">
                <span
                  className="inline-flex items-center gap-1 text-[11px] font-bold font-headline px-2 py-0.5 rounded-full"
                  style={{ backgroundColor: "rgba(21,128,61,0.10)", color: "#15803d" }}
                >
                  ✓ Signed
                </span>
                {s.signedAt && (
                  <p className="text-[11px] mt-1.5" style={{ color: MUTED }}>
                    {fmtDateTime(s.signedAt, timeZone)}
                  </p>
                )}
              </div>
            </div>
      ...
      {/* Record — provenance + tamper-evidence. */}
          ...
          {sha256 && (
            <>
              ...
              <p className="mt-1" style={{ color: FAINT }}>
                Tamper-evident fingerprint — any change to the agreement would
