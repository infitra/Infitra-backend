...
export default async function DashboardPage() {
  ...
  // ── The Pulse: what needs attention right now ──────────
  ...
  return (
    <div className="py-6">
          ...
          <div className="p-6">
            <div className="flex items-center justify-between">
              ...
              <Link
                href={
                  liveSession
                    ? `/dashboard/sessions/${liveSession.id}/live`
                    : `/dashboard/sessions/${pulseSession.id}`
                }
                ...
                style={{
                  ...
                  boxShadow: liveSession || goLiveSession
                    ? "0 4px 14px rgba(255,97,48,0.35)"
                    : "0 4px 14px rgba(15,34,41,0.15)",
                }}
              >
      ...
      {/* ── ECOSYSTEM: Community + Tribes ──────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">

        {/* ── HOME BASE: Community (3 cols) ────────────────── */}
          ...
          <div className="rounded-2xl infitra-card overflow-hidden">
            ...
            <div className="px-6 pt-5 pb-4 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-sm font-bold font-headline text-[#0F2229] uppercase tracking-wider">
                  Home Base
                </h2>
        ...
        {/* ── RIGHT SIDE: Tribes + Sessions (2 cols) ────────── */}
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold font-headline text-[#0F2229] uppercase tracking-wider">Tribes</h2>
              </div>
              <form action={createDraftChallenge}>
                <button type="submit" className="text-[10px] font-bold font-headline text-[#FF6130]">
                  + New Challenge
                </button>
              </form>
            </div>
            ...
            {hasTribes ? (
                ...
                {challengeSpaces!.map((cs: any) => {
                  ...
                  return (
                    <Link key={cs.id} href={`/communities/challenge/${cs.id}`} className="group block rounded-2xl infitra-card-link overflow-hidden">
                      <div className="h-0.5" style={{ background: "linear-gradient(90deg, #FF6130, rgba(255,97,48,0.2))" }} />
                      <div className="p-4">
                        <div className="flex items-center justify-between mb-2">
                          <h3 className="text-sm font-black font-headline tracking-tight truncate text-[#0F2229] group-hover:text-[#FF6130]">
                            {cs.title}
                          </h3>
                          <span className="text-[10px] text-[#94a3b8] shrink-0 ml-2">{mCount} mbr</span>
                        </div>
                        {nextSess && (
                          <div className="flex items-center gap-1.5 text-[10px]">
                            <span className="w-1.5 h-1.5 rounded-full bg-[#0891b2]" />
                            <span className="text-[#0891b2] font-bold font-headline">{formatRelativeTime(nextSess.start_time)}</span>
                            <span className="text-[#64748b] truncate">· {nextSess.title}</span>
                          </div>
                        )}
                        {!nextSess && (
              ...
              <div className="p-5 rounded-2xl border border-dashed text-center" style={{ borderColor: "rgba(15,34,41,0.12)" }}>
                <p className="text-xs text-[#64748b] mb-1">No tribes yet</p>
                <p className="text-[10px] text-[#94a3b8]">Publish a challenge to create your first tribe</p>
              </div>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-[#0891b2]" />
                <h2 className="text-sm font-bold font-headline text-[#0F2229] uppercase tracking-wider">Sessions</h2>
              </div>
              <Link href="/dashboard/sessions/new" className="text-[10px] font-bold font-headline text-[#FF6130]">
                + New Session
              </Link>
            </div>
            ...
            {(upcomingSessions ?? []).length > 0 ? (
              <div className="space-y-2">
                {upcomingSessions!.slice(liveSession || goLiveSession ? 1 : 0).map((sess: any) => {
                  ...
                  return (
                    <Link key={sess.id} href={`/dashboard/sessions/${sess.id}`} className="group flex items-center justify-between p-3 rounded-xl infitra-card-link">
                      ...
                      {hasRoom && (
                        <span className="px-2.5 py-1 rounded-full bg-[#FF6130] text-white text-[9px] font-bold font-headline shrink-0">Live</span>
                      )}
                    </Link>
                  );
                })}
              ...
              <div className="p-4 rounded-2xl border border-dashed text-center" style={{ borderColor: "rgba(15,34,41,0.12)" }}>
                <p className="text-xs text-[#64748b]">No upcoming sessions</p>
              </div>
            )}
