 ...
 * not ours, and the agreement records it before anything sells.
 *
 * It sits next to the ask rather than up in the pitch, because a split is
 * what a decider looks for and a question nobody has asked yet is load.
...
export function SplitVisual() {
  return (
    <div className="rounded-3xl p-5 sm:p-7" style={{ backgroundColor: "#FFFFFF", boxShadow: CARD_SHADOW }}>
      <p className="text-[11px] uppercase tracking-[0.25em] font-headline" style={{ color: CYAN, fontWeight: 700 }}>
        One sale
      </p>
      ...
      <div className="mt-4 flex flex-col sm:flex-row rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(15,34,41,0.10)" }}>
        <div
          className="w-full sm:w-[90%] shrink-0 flex items-stretch"
          style={{ background: "linear-gradient(120deg, rgba(255,97,48,0.12) 0%, rgba(8,145,178,0.12) 100%)" }}
        >
          ...
          <div className="self-stretch my-3 shrink-0" style={{ borderLeft: "2px dashed rgba(15,34,41,0.26)" }} />
          <div className="flex-1 px-4 py-4 sm:py-5 text-left flex flex-col justify-center">
      ...
      <div className="mt-6 pt-6" style={{ borderTop: "1px solid rgba(15,34,41,0.10)" }}>
        <p data-independence className="text-lg md:text-xl leading-relaxed font-headline" style={{ color: INK, fontWeight: 600, letterSpacing: "-0.01em" }}>
          You stay independent and still work together professionally.
        </p>
        <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-2.5">
          {KEEPS.map((k) => (
            <span key={k.label} className="inline-flex items-start gap-2 text-[14px] leading-snug" style={{ color: MUTED }}>
